...
static func text_to_dictionary(text: String) -> Dictionary:
	...
	if error == OK:
		...
		if typeof(data_received) != TYPE_DICTIONARY:
			InkUtils.throw_exception("Incorrect data type! Expected TYPE_DICTIONARY but it is %s" % typeof(data_received))
			return {}

		return data_received
	else:
		InkUtils.throw_exception("Error parsing JSON data! %s" % error)
		return {}
...
static func text_to_array(text: String) -> Array:
	...
	if error == OK:
		...
		if typeof(data_received) != TYPE_ARRAY:
			InkUtils.throw_exception("Incorrect data type! Expected TYPE_ARRAY but it is %s" % typeof(data_received))
			return []
	...
	func read_number():
		...
		if is_float:
			if num_str.is_valid_float():
				push_error("FLOAT: ", num_str)
				return float(num_str)
		else:
			if num_str.is_valid_int():
				push_error("INT: ", num_str)
				return int(num_str)
