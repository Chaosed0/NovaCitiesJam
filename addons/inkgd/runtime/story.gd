...
func _init(json_string: String, runtime = null):
	...
	print("InkStory.init format: ", format_from_file)
	...
	print("InkStory.init root: ", root_token)
	if root_token == null:
		InkUtils.throw_exception(
	...
	if root_object.has("listDefs"):
		print("InkStory.init listdefs pre")
		self._list_definitions = self.StaticJSON.jtoken_to_list_definitions(root_object["listDefs"])
		print("InkStory.init listdefs post ", _list_definitions)

	print("InkStory.init main content container pre")
	self._main_content_container = InkUtils.as_or_null(
			self.StaticJSON.jtoken_to_runtime_object(root_token),
			"InkContainer"
	)
	print("InkStory.init main content container post ", _main_content_container)

	print("reset state pre")
	self.reset_state()
	print("reset state post")
...
func next_sequence_shuffle_index() -> int:
	var eval_stack = self.state.pop_evaluation_stack()
	var num_elements_int_val = InkUtils.as_or_null(eval_stack, "IntValue")
