...
func _finalise_story_creation() -> void:
	...
	if ink_runtime == null:
		_push_null_runtime_error()
		print("emit loaded false")
		emit_signal("loaded", false)
		return

	print("emit loaded true")
...
func _current_platform_supports_threads() -> bool:
	print("supports threads ", OS.get_name())
	return OS.get_name() != "HTML5" && OS.get_name() != "Web"
